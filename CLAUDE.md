...
### 作業完了時の要件
作業を終了する際は、以下の状態で完了すること:
- **ビルドエラーがないこと** — `mvnw.cmd clean compile` と `build\build.bat` の両方が成功すること
- **警告がないこと** — IDE での未使用インポートやコンパイル警告がないこと
- **テストが全てパスしていること** — `mvnw.cmd test` で全テストが成功すること
- **コミット・プッシュが完了していること** — PR を作成する直前まで全ての変更がコミット済みであること

ビルドエラーや警告を残したまま作業を終了しない。特に以下に注意:

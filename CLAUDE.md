...
### 判断基準：「有用性」ではなく「置き場所」
コメントが冗長になりやすいのは、情報そのものが不要だからではなく、**書く場所を誤っている**ことが多い。「そのコメントが有用か」ではなく「その情報をどこに置くべきか」で判断すること。情報は次の3層に分けて配置する:

| 層 | 内容 | 置くべき情報 |
|---|---|---|
| コード本文 | 今すぐ動作判断に必要な情報 | コードから復元できず、かつ今すぐ必要なものだけ |
| コミットメッセージ・Issue | 変更の経緯・背景 | 「なぜ変更したか」「何を試して何をやめたか」 |
| CLAUDE.md・ルールファイル | 繰り返し学んだ教訓 | ツールの既定動作と異なる、プロジェクト固有の慣習 |

**削除テスト**: コメント行を書いたら、それを消したときに AI やレビュアーが実際に困るかを自問する。困らなければ削除する。

### コメントを書かない条件（共通）
過剰なコメントは読み手の負担になるだけでなく、AI にとってはコンテキストを圧迫し後続の判断精度を落とす要因にもなるため、以下の場合はコメントを書かない:
- **コードを読めば自明な内容**（変数名・メソッド名・型で意図が分かるもの）
- **処理の「何を」を繰り返すだけのコメント**（`// カウンター変数をインクリメント` など）
- **変更履歴**（「〜を追加した」「〜を修正した」など） — git log・コミットメッセージに委ねる
- **タスク・Issue 番号の参照**（「Issue #123 対応」など） — Issue 側に委ねる。コードは常に現在の状態のみを語る
